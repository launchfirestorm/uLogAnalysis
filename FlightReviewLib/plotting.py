...
from helper import (
    ...
    project_latlon_on_map, map_projection_from_ulog
    )
...
def get_mean_gap(ref_time, ref_data, time, data):
    """ Gets the mean gap between two signals and the corresponding RMS; used to align altitude data better """
    data_interp = np.interp(ref_time, time, data)
    diff = data_interp - ref_data
    mean_diff = np.mean(diff)
    return (mean_diff, np.sqrt(np.mean((diff - mean_diff)**2)), diff)
...
class DataPlotFFT(DataPlot):
    ...
    def add_graph(self, field_names, colors, legends):
        ...
        try:
            ...
            if sampling_frequency < 15 or sampling_frequency == float("inf"): # require min sampling freq
                self._had_error = True
                return
...
def altitude_profile_plot(ulog, config, plot_title, x_label, y_label):
    gps_t = ulog.get_dataset('vehicle_gps_position').data['timestamp'][1:]
    gps_x, gps_y = map_projection_from_ulog(ulog)
    gps_ds = np.sqrt((np.diff(gps_x))**2 + (np.diff(gps_y))**2)
    ground_course = np.cumsum(gps_ds)
    
    
    alt_z = ulog.get_dataset('vehicle_local_position').data['z']
    alt_t = ulog.get_dataset('vehicle_local_position').data['timestamp']
    z = -(alt_z - alt_z[0])
    
    dist_bottom = ulog.get_dataset('vehicle_local_position').data['dist_bottom']
    
    initial_abs_height = ulog.get_dataset('vehicle_global_position').data['alt'][0]
    tecs_t      = ulog.get_dataset('tecs_status').data['timestamp']
    tecs_h_sp   = ulog.get_dataset('tecs_status').data['altitude_sp'] - initial_abs_height
    
    ground_course_interp = interpolate.interp1d(gps_t, ground_course, kind = 'previous', fill_value = "extrapolate")
    
    plot = figure(plot_height = config['plot_height']['normal'], plot_width = config['plot_width'],
                  title = plot_title, x_axis_label = x_label, y_axis_label = y_label)
    
    plot.line(ground_course_interp(alt_t), z, color='blue',legend='estimated altitude', line_width = 3)
    plot.line(ground_course_interp(alt_t), dist_bottom, color='red',legend='distance to bottom', line_width = 3)
    plot.line(ground_course_interp(tecs_t), tecs_h_sp, color='green',legend='TECS Height Setpoint', line_width = 3)
