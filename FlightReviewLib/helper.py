...
def load_ulog_file(file_name):
    ...
    msg_filter = ['battery_status', 'distance_sensor', 'estimator_status',
                  ...
                  'failsafe_flags',
                  'mission_result', 'home_position', 'position_controller_status',
                  'estimator_wind', 'vehicle_land_detected', 'position_controller_landing_status',
                  'kingtech_g4p_report', 'satellite_info', 'estimator_innovation_test_ratios',
                  'estimator_innovations', 'estimator_innovation_variances', 'estimator_states',
                  'fuze_control_status', 'ping', 'radio_status', 'fuel_tank_status', 'sensor_mag',
                  'power_monitor', 'flaps_setpoint', 'spoilers_setpoint', 'landing_gear_wheel', 
                  'brakes_setpoint', 'npfg_status', 'wind', 'airspeed_wind', 'uavcan_status']
    try:
...
def map_projection_from_ulog(ulog):
    ...
    try:
        ...
        pass
    x, y = map_projection(lat, lon, anchor_lat, anchor_lon)
    
    return x, y

def project_latlon_on_map(ulog, lat_rad, lon_rad):
    cur_dataset = ulog.get_dataset('vehicle_gps_position')
    t = cur_dataset.data['timestamp']
    indices = cur_dataset.data['fix_type'] > 2 # use only data with a fix
    t = t[indices]
    ...
    try:
        ...
        pass
    x, y = map_projection(lat_rad, lon_rad, anchor_lat, anchor_lon)
    
    return x, y

def rotate_FRD_to_NED(ulog, t, x, y, z):
    att_data = ulog.get_dataset('vehicle_attitude')
    phi     = np.interp(t, att_data.data['timestamp'], att_data.data['roll'])
    theta   = np.interp(t, att_data.data['timestamp'], att_data.data['pitch'])
    psi     = np.interp(t, att_data.data['timestamp'], att_data.data['yaw'])
    
    x_rot = z * (np.sin(phi) * np.sin(psi) + np.cos(phi) * np.cos(psi) * np.sin(theta)) - y * (np.cos(phi) * np.sin(psi) - np.cos(psi) * np.sin(phi) * np.sin(theta)) + x * np.cos(psi) * np.cos(theta)
    y_rot = y * (np.cos(phi) * np.cos(psi) + np.sin(phi) * np.sin(psi) * np.sin(theta)) - z * (np.cos(psi) * np.sin(phi) - np.cos(phi) * np.sin(psi) * np.sin(theta)) + x * np.cos(theta) * np.sin(psi)
    z_rot = z * np.cos(phi) * np.cos(theta) - x * np.sin(theta) + y * np.cos(theta) * np.sin(phi)
    
    return x_rot, y_rot, z_rot

def rotate_NED_to_FRD(q, vec):
    ...
    # Transform the relative velocity into the body frame
    vec_rot = np.dot(R.transpose(), vec)  
    
    return vec_rot

def compute_aero_angles(V_aircraft, V_wind, q):
    # Compute the relative velocity
    V_relative = np.array(V_aircraft) - np.array(V_wind)
    
    # Don't compute when the plane is moving too slowly (10 [m/s] = 22.3 [mph])
    if np.linalg.norm(V_relative) > 10:
        V_body = rotate_NED_to_FRD(q, V_relative)
        
        # Compute the angle of attack and side slip angle
        AoA = np.arctan2(V_body[2], V_body[0])
        SSA = np.arctan2(V_body[1], V_body[0])
    else:
        AoA = 0.0
        SSA = 0.0
...
def compute_aero_angles_log(ulog):
    att = ulog.get_dataset('vehicle_attitude')
    t   = att.data['timestamp']
    q0  = att.data['q[0]']
    q1  = att.data['q[1]']
    q2  = att.data['q[2]']
    q3  = att.data['q[3]']
    
    v_pos       = ulog.get_dataset('vehicle_local_position')
    v_pos_t     = v_pos.data['timestamp']
    vx          = np.interp(t, v_pos_t, v_pos.data['vx'])
    vy          = np.interp(t, v_pos_t, v_pos.data['vy'])
    vz          = np.interp(t, v_pos_t, v_pos.data['vz'])
    
    for i in range(5):
        try:
            w_est       = ulog.get_dataset('estimator_wind', i)
            break
        except Exception as e:
            raise ValueError("estimator_wind instance " + str(i) + " does not exist" )
           
    try:
        w_est_t     = w_est.data['timestamp']
        wx          = np.interp(t, w_est_t, w_est.data['windspeed_north'])
        wy          = np.interp(t, w_est_t, w_est.data['windspeed_east'])
        wz          = np.zeros(np.size(t))
        
        AoA_t = np.zeros(np.size(t))
        SSA_t = np.zeros(np.size(t))
        AoA_noWind_t = np.zeros(np.size(t))
        SSA_noWind_t = np.zeros(np.size(t))
        
        for i in range(len(t)):
            vehicle_vel = [vx[i], vy[i], vz[i]]
            wind_vel    = [wx[i], wy[i], wz[i]]
            q           = [q0[i], q1[i], q2[i], q3[i]]
            AoA, SSA                = compute_aero_angles(vehicle_vel, wind_vel, q)
            AoA_noWind, SSA_noWind  = compute_aero_angles(vehicle_vel, [0,0,0], q)
            AoA_t[i]                = AoA
            SSA_t[i]                = SSA
            AoA_noWind_t[i]            = AoA_noWind
            SSA_noWind_t[i]            = SSA_noWind
        
        return AoA_t, SSA_t, AoA_noWind_t, SSA_noWind_t
    except Exception as e:
        raise ValueError('no estimator_wind instance exists')
